/**
 * LeanConstraintsContent — Embeddable content for Lean Constraints.
 * Can be used standalone (in LeanConstraints page) or embedded (in LPSModule tab).
 */
...
import { Shield, Plus, Loader2, FileBarChart, ClipboardCheck } from 'lucide-react';
...
export function LeanConstraintsContent() {
  ...
  const formatMutationError = (e: Error) => `Erro: ${e.message || 'Erro desconhecido'}`;

  const handleCreate = (data: ConstraintPayload) => {
    createConstraint.mutate(data, {
      onSuccess: (result) => {
        toast.success('Restrição criada com sucesso');
        addHistoryEntry({ constraint_id: result?.id || '', action: 'created', user_name: userName });
  ...
  const handleUpdate = (data: ConstraintPayload) => {
    ...
    updateConstraint.mutate({ id: editingConstraint.id, ...data }, {
      onSuccess: () => {
        ...
        addHistoryEntry({ constraint_id: editingConstraint.id, action: 'updated', user_name: userName });
  ...
  const confirmResolve = () => {
    ...
    resolveConstraint.mutate(resolveTarget, {
      onSuccess: () => {
        ...
        addHistoryEntry({ constraint_id: resolveTarget, action: 'resolved', user_name: userName });
  ...
  const confirmDelete = () => {
    ...
    deleteConstraint.mutate(deleteTarget, {
      onSuccess: () => {
        ...
        addHistoryEntry({ constraint_id: deleteTarget, action: 'deleted', user_name: userName });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold flex items-center gap-2">
            <Shield className="h-5 w-5 text-indigo-600" /> Restrições Lean
          </h2>
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <Button size="sm" onClick={() => { setEditingConstraint(null); setMapCoordinates(null); setCreateOpen(true); }}>
      ...
      <DeadlineNotifications
        ...
        onGoToConstraint={(c) => { setEditingConstraint(c); setCreateOpen(true); }}
      />
      ...
      <Tabs defaultValue="lista">
        ...
        <TabsContent value="mapa" className="mt-4">
          <ConstraintMapLayer constraints={constraints} onCreateAtLocation={handleMapCreate} />
        </TabsContent>
      ...
      <WeeklyReportDialog open={reportOpen} onOpenChange={setReportOpen} report={weeklyReport} projectName={currentProjectName} />
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tem certeza que deseja marcar esta restrição como resolvida?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(o) => { if (!o) setDeleteTarget(null); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta ação não pode ser desfeita. A restrição será permanentemente removida.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancelar</AlertDialogCancel>
