...
const HydroNetworkLanding = () => {
  ...
  return (
    <div className="min-h-screen" style={{ background: "#F5F5F5", color: "#0E1B3D", fontFamily: "'Inter', system-ui, sans-serif" }}>
      {/* Header */}
      <header className="fixed top-0 left-0 right-0 z-50 backdrop-blur-md border-b" style={{ background: "rgba(245,245,245,0.95)", borderColor: "#D4D4D4" }}>
        <div className="max-w-7xl mx-auto px-3 sm:px-4 h-14 sm:h-16 flex items-center justify-between">
          <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
            <img src="/logo.svg" alt="ConstruData" className="h-6 sm:h-8" />
            <span className="text-sm sm:text-lg font-bold font-mono hidden xs:inline" style={{ color: "#10367D" }}>
              <LogoText className="text-sm sm:text-lg" textColor="text-[#10367D]" /> <span style={{ color: "#FF6B2C" }}>HydroNetwork</span>
            </span>
          </div>
          ...
          <Button onClick={() => navigate("/hydronetwork")} className="font-semibold text-xs sm:text-sm px-3 sm:px-4" style={{ background: "#FF6B2C", color: "#FFFFFF" }}>
            ACESSAR
          </Button>
        </div>
      </header>
      ...
      {/* Hero */}
      <section className="relative min-h-screen flex items-center justify-center pt-14 sm:pt-16" style={{ background: "linear-gradient(135deg, #0A2456 0%, #10367D 50%, #1A4A9E 100%)" }}>
        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 text-center space-y-5 sm:space-y-8 py-8 sm:py-0">
          <Badge className="text-[10px] sm:text-sm px-3 sm:px-4 py-1" style={{ background: "rgba(255,107,44,0.15)", color: "#FF6B2C", border: "1px solid rgba(255,107,44,0.3)" }}>
            PLATAFORMA COMPLETA DE ENGENHARIA DE SANEAMENTO
          </Badge>
          <h1 className="text-3xl sm:text-5xl md:text-7xl font-bold font-mono text-white">
            CONSTRUDATA
            <br />
            <span style={{ color: "#FF6B2C", fontSize: "0.7em" }}>HYDRONETWORK</span>
          </h1>
          <p className="text-sm sm:text-lg md:text-xl max-w-4xl mx-auto px-2" style={{ color: "#94A3B8" }}>
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center px-4 sm:px-0">
            <Button size="lg" onClick={() => navigate("/hydronetwork")} className="font-bold text-sm sm:text-lg px-6 sm:px-8 w-full sm:w-auto" style={{ background: "#FF6B2C", color: "#FFFFFF" }}>
              Acesso Gratuito — DEMO <ArrowRight className="ml-2 h-4 sm:h-5 w-4 sm:w-5" />
            </Button>
            <Button size="lg" variant="outline" onClick={() => document.getElementById("modulos")?.scrollIntoView({ behavior: "smooth" })}
              className="font-bold text-sm sm:text-lg px-6 sm:px-8 w-full sm:w-auto" style={{ borderColor: "#FF6B2C", color: "#FF6B2C" }}>
              Ver Módulos
            </Button>
            <Button size="lg" variant="outline" onClick={() => window.open('/hub/index.html', '_blank')}
              className="font-bold text-sm sm:text-lg px-6 sm:px-8 w-full sm:w-auto" style={{ borderColor: "#1A4A9E", color: "#FF6B2C" }}>
              <Newspaper className="mr-2 h-4 sm:h-5 w-4 sm:w-5" /> Hub de Notícias
            </Button>
          </div>

          {/* Verified metrics */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-4 mt-8 sm:mt-12">
            {[
              ...
              <div key={i} className="rounded-none p-3 sm:p-4 text-center" style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)" }}>
                <div className="text-2xl sm:text-3xl font-bold" style={{ color: "#FF6B2C" }}>{f.v}</div>
                <div className="text-[10px] sm:text-xs mt-1" style={{ color: "#94A3B8" }}>{f.l}</div>
              </div>
            ))}
          </div>

          <a href="#screenshots" className="inline-block mt-6 sm:mt-8 animate-bounce">
            <ChevronDown className="h-6 sm:h-8 w-6 sm:w-8" style={{ color: "#FF6B2C" }} />
          </a>
        </div>
      </section>

      {/* Screenshots Section */}
      <section id="screenshots" data-animate className={`py-12 sm:py-24 px-4 transition-all duration-700 ${isVisible("screenshots") ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`} style={{ background: "#F5F5F5" }}>
        <div className="max-w-6xl mx-auto space-y-8 sm:space-y-12">
          <div className="text-center space-y-3 sm:space-y-4">
            <h2 className="text-2xl sm:text-4xl font-bold font-mono" style={{ color: "#10367D" }}>Veja a plataforma em ação</h2>
            <p className="text-sm sm:text-base" style={{ color: "#64748B" }}>Ferramentas profissionais direto no navegador</p>
          </div>
      ...
      {/* Cost Map Section */}
      <section id="custos" data-animate className={`py-12 sm:py-24 px-4 transition-all duration-700 ${isVisible("custos") ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`} style={{ background: "#0A2456" }}>
        <div className="max-w-6xl mx-auto space-y-8 sm:space-y-12">
          <div className="text-center space-y-3 sm:space-y-4">
            <Badge className="text-[10px] sm:text-sm" style={{ background: "rgba(255,107,44,0.15)", color: "#FF6B2C", border: "1px solid rgba(255,107,44,0.3)" }}>Controle Financeiro Total</Badge>
            <h2 className="text-2xl sm:text-4xl md:text-5xl font-bold font-mono text-white">Previsão de Custos por Trecho no Mapa</h2>
            <p className="text-sm sm:text-lg max-w-2xl mx-auto" style={{ color: "#94A3B8" }}>Saiba exatamente quanto cada metro de rede vai custar antes de iniciar.</p>
          </div>
      ...
      <section data-animate id="workflow" className={`py-12 sm:py-24 px-4 transition-all duration-700 ${isVisible("workflow") ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
        style={{ background: "#F5F5F5" }}>
        <div className="max-w-6xl mx-auto space-y-8 sm:space-y-12">
          <div className="text-center space-y-3 sm:space-y-4">
            <h2 className="text-2xl sm:text-4xl font-bold font-mono" style={{ color: "#10367D" }}>Fluxo Completo em 8 Etapas</h2>
            <p className="text-sm sm:text-base" style={{ color: "#64748B" }}>Do projeto à entrega, tudo integrado</p>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-4">
            {workflow.map((step, i) => (
              <div key={i} className="rounded-none p-3 sm:p-6 text-center space-y-1 sm:space-y-2 hover:scale-105 transition-transform" style={{ background: "#FFFFFF", border: "1px solid #EBEBEB" }}>
                <div className="text-xl sm:text-2xl font-bold" style={{ color: "#FF6B2C" }}>{step.step}</div>
                <h3 className="text-xs sm:text-sm font-bold" style={{ color: "#10367D" }}>{step.label}</h3>
                <p className="text-[10px] sm:text-xs" style={{ color: "#64748B" }}>{step.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Normas Técnicas */}
      <section id="normas" data-animate className={`py-12 sm:py-24 px-4 transition-all duration-700 ${isVisible("normas") ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`} style={{ background: "#0A2456" }}>
        <div className="max-w-6xl mx-auto space-y-6 sm:space-y-8">
          <div className="text-center space-y-3 sm:space-y-4">
            <h2 className="text-2xl sm:text-4xl font-bold font-mono text-white">Conforme Normas Brasileiras</h2>
            <p className="text-sm sm:text-base" style={{ color: "#94A3B8" }}>Cálculos validados conforme as principais normas de saneamento</p>
          </div>
      ...
      {/* Modules */}
      <section id="modulos" data-animate className={`py-12 sm:py-24 px-4 transition-all duration-700 ${isVisible("modulos") ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
        style={{ background: "#F5F5F5" }}>
        <div className="max-w-6xl mx-auto space-y-8 sm:space-y-12">
          <div className="text-center space-y-3 sm:space-y-4">
            <h2 className="text-2xl sm:text-4xl font-bold font-mono" style={{ color: "#10367D" }}>17 Módulos Integrados</h2>
            <p className="text-sm sm:text-base" style={{ color: "#64748B" }}>Tudo que você precisa em uma única plataforma</p>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-4">
            {modules.map((m, i) => (
              <Card key={i} className={`hover:scale-105 transition-transform border-0 relative ${m.badge ? "opacity-70" : ""}`} style={{ background: "#FFFFFF", border: "1px solid #EBEBEB" }}>
                {m.badge && (
                  <div className="absolute top-1.5 right-1.5 sm:top-2 sm:right-2 px-1.5 sm:px-2 py-0.5 rounded text-[8px] sm:text-[10px] font-bold" style={{ background: "rgba(245,158,11,0.1)", color: "#F59E0B" }}>{m.badge}</div>
                )}
                <CardContent className="pt-4 sm:pt-6 text-center space-y-1 sm:space-y-2">
                  <m.icon className="h-6 w-6 sm:h-8 sm:w-8 mx-auto" style={{ color: m.color }} />
                  <h3 className="text-[11px] sm:text-sm font-bold" style={{ color: "#10367D" }}>{m.label}</h3>
                  <p className="text-[10px] sm:text-xs" style={{ color: "#64748B" }}>{m.desc}</p>
                </CardContent>
              </Card>
            ))}
          </div>
      ...
      {/* Social Proof */}
      <section data-animate id="social" className={`py-12 sm:py-24 px-4 transition-all duration-700 ${isVisible("social") ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`} style={{ background: "#0A2456" }}>
        <div className="max-w-6xl mx-auto space-y-8 sm:space-y-12">
          <div className="text-center space-y-3 sm:space-y-4">
            <h2 className="text-2xl sm:text-4xl font-bold font-mono text-white">Projetado por engenheiros, para engenheiros</h2>
          </div>
      ...
      <section data-animate id="construdata" className={`py-10 sm:py-16 px-4 transition-all duration-700 ${isVisible("construdata") ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
        style={{ background: "#F5F5F5" }}>
        <div className="max-w-5xl mx-auto text-center space-y-4 sm:space-y-6">
          <p className="text-xs sm:text-sm" style={{ color: "#64748B" }}>Conheça também nossa plataforma de gestão de obras</p>
          <h2 className="text-xl sm:text-2xl font-bold font-mono" style={{ color: "#10367D" }}>CONSTRUDATA Obras</h2>
      ...
      {/* CTA Final */}
      <section className="py-12 sm:py-24 px-4 text-center" style={{ background: "linear-gradient(135deg, #0A2456 0%, #10367D 100%)" }}>
        <div className="max-w-2xl mx-auto space-y-4 sm:space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center px-4 sm:px-0">
            <Button size="lg" onClick={() => navigate("/hydronetwork")} className="font-bold text-sm sm:text-lg px-6 sm:px-8 w-full sm:w-auto" style={{ background: "#FF6B2C", color: "#FFFFFF" }}>
              Acessar Gratuitamente <ArrowRight className="ml-2 h-4 sm:h-5 w-4 sm:w-5" />
            </Button>
            <Button size="lg" variant="outline" onClick={() => navigate("/hydronetwork")} className="w-full sm:w-auto" style={{ borderColor: "#FF6B2C", color: "#FF6B2C" }}>
              Carregar Projeto Exemplo
            </Button>
          </div>
          <p className="text-xs sm:text-sm" style={{ color: "#94A3B8" }}>
            Sem cadastro obrigatório &bull; Sem cartão de crédito &bull; Plano DEMO gratuito
          </p>
        </div>
      </section>

      {/* Footer */}
      <footer className="py-8 sm:py-12 px-4" style={{ background: "#0A2456", borderTop: "1px solid #1E3A6E" }}>
        <div className="max-w-6xl mx-auto grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6 sm:gap-8 text-sm" style={{ color: "#94A3B8" }}>
          <div className="col-span-2 sm:col-span-1">
            <div className="flex items-center gap-2 mb-3 sm:mb-4">
              <img src="/logo-white.svg" alt="ConstruData" className="h-5 sm:h-6" />
              <span className="font-bold text-white text-xs sm:text-sm">HYDRONETWORK</span>
            </div>
            <p className="text-[10px] sm:text-xs">Plataforma de engenharia de saneamento</p>
          </div>
          <div>
            <h4 className="font-bold text-white mb-2 sm:mb-3 text-xs sm:text-sm">Módulos</h4>
            {["Topografia", "Esgoto", "Água", "Drenagem"].map(m => <p key={m} className="text-[10px] sm:text-xs mb-1">{m}</p>)}
          </div>
          <div>
            <h4 className="font-bold text-white mb-2 sm:mb-3 text-xs sm:text-sm">Ferramentas</h4>
            {["EPANET PRO", "QGIS", "Gantt", "Curva S"].map(m => <p key={m} className="text-[10px] sm:text-xs mb-1">{m}</p>)}
          </div>
          <div>
            <h4 className="font-bold text-white mb-2 sm:mb-3 text-xs sm:text-sm">Formatos</h4>
            {["CSV/Excel", "DXF/SHP", "GeoJSON/KML", "PDF"].map(m => <p key={m} className="text-[10px] sm:text-xs mb-1">{m}</p>)}
          </div>
          <div>
            <h4 className="font-bold text-white mb-2 sm:mb-3 text-xs sm:text-sm">Normas</h4>
            {["NBR 9649", "NBR 12218", "SINAPI", "SICRO"].map(m => <p key={m} className="text-[10px] sm:text-xs mb-1">{m}</p>)}
          </div>
        </div>
        <div className="max-w-6xl mx-auto mt-6 sm:mt-8 pt-6 sm:pt-8 text-center text-[10px] sm:text-xs" style={{ borderTop: "1px solid #1E3A6E", color: "#64748B" }}>
          © 2026 CONSTRUDATA HydroNetwork — construdata.software — Todos os direitos reservados.
        </div>
      </footer>
    </div>
  );
